...
def filter_by_state(my_list: List, my_state='EXECUTED') -> List:
    """
...
def sort_by_date(my_list: List, descending=True) -> List:
    """
    Функция сортирует исходный список по значению параметра 'date'
    Направление сортировки задается параметром 'ascending'
    Если значение параметра True или параметр не задан, но сортировка по убыванию.
